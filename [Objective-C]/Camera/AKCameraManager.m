...
- (AVCaptureDevice *)frontCamera {
    NSArray *devices = [[AVCaptureDeviceDiscoverySession discoverySessionWithDeviceTypes: @[AVCaptureDeviceTypeBuiltInWideAngleCamera, AVCaptureDeviceTypeBuiltInTelephotoCamera, AVCaptureDeviceTypeBuiltInDualCamera] mediaType:AVMediaTypeVideo position:AVCaptureDevicePositionFront] devices];
...
- (AVCaptureDevice *)backCamera {
    NSArray *devices = [[AVCaptureDeviceDiscoverySession discoverySessionWithDeviceTypes:@[AVCaptureDeviceTypeBuiltInWideAngleCamera,AVCaptureDeviceTypeBuiltInTelephotoCamera,AVCaptureDeviceTypeBuiltInDualCamera] mediaType:AVMediaTypeVideo position:AVCaptureDevicePositionBack] devices];
    AVCaptureDevice * camera = devices.firstObject;
    if ([camera isFocusModeSupported:AVCaptureFocusModeContinuousAutoFocus]) {
        NSError *error;
        [camera lockForConfiguration:&error];
...
- (AVCaptureVideoPreviewLayer *)setVideoLayer:(CALayer *)layer {
    ...
    if (self.captureVideoPreviewLayer) {
        ...
        [self.captureVideoPreviewLayer setFrame:layer.bounds];
        [layer insertSublayer: self.captureVideoPreviewLayer atIndex:0];
        self.captureVideoPreviewLayer.zPosition = -1;
    }
